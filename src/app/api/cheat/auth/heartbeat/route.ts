// POST /api/cheat/auth/heartbeat
// body: { sessionToken, hwid, product? }
// 200: { ok: true, expiresAt, nextHeartbeatMs, offsets: { product, version, keyId, data, signature } }
...
import { HEARTBEAT_RATE_LIMIT, rateLimitByIp } from "@/lib/cheat/rate-limit";
import { heartbeat } from "@/server/services/cheat/cheat-auth-service";
...
export async function POST(req: Request) {
  ...
  const rl = rateLimitByIp(
    ip,
    HEARTBEAT_RATE_LIMIT.maxAttempts,
    HEARTBEAT_RATE_LIMIT.windowMs,
  );
  if (!rl.allowed) {
